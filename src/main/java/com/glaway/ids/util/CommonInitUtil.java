...
public class CommonInitUtil {
    ...
    public static void initGLObjectForCreate(GLObject data,TSUserDto curUser,String orgId) {
        if(CommonUtil.isEmpty(curUser)){
        ...
        if (StringUtils.isNotBlank(orgId)) {
            data.setCreateOrgId(orgId);
        }
    }

    public static void initGLObjectForUpdate(GLObject data,TSUserDto curUser,String orgId) {
        if(CommonUtil.isEmpty(curUser)){
            curUser = UserUtil.getCurrentUser();
        }
        data.setUpdateBy(curUser.getId());
